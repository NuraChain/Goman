...
# The node binary, not NODE_PATH: that name is Node's own module-resolution variable, and a box
# that has it set for unrelated reasons would otherwise be read as an interpreter path here.
...
# No build step for the server - Node >= 24 runs the TypeScript source directly. WorkingDirectory
# is the SERVER directory, not the repo root: main.ts reads `.env` from the working directory, and
# CLIENT_DIR/UPLOAD_DIR are resolved relative to it.
...
# The CLIENT half does have a build step, and in production this server serves that directory as
# static files - without it the API answers and every page is a 404. Say so here rather than in a
# support thread later.
if [ ! -f "$ROOT/application/dist/index.html" ]; then
  echo "warning: application/dist/index.html is missing - run 'npm run build' before starting" >&2
fi
...
# server serves no client and attaches the dev-only wiring. The file below overrides.
...
ExecStart="$NODE_BIN" $SERVICE_PATH_APP
