...
import Tooltip from '../src/components/ui/tooltip.azeroth';
...
describe('Tooltip', () =>
{
    ...
    {
        render(() => Tooltip({ label: 'Copy link', children: ['x'] }));
        expect(bubble()).toBeNull();
    });

    it('portals the bubble OUT of the trigger subtree, onto the body', () =>
    {
        const { container } = render(() => Tooltip({ label: 'Copy link', children: ['x'] }));
