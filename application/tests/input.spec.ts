...
import { render, cleanup } from './harness.ts';

import Input from '../src/components/ui/input.azeroth';
...
describe('Input direction', () =>
{
    ...
    {
        const { container } = render(() => Input({ label: 'Category', dir: 'ltr', placeholder: 'برای جستجو تایپ کن، یا دسته جدید بساز', value: '' }));
        expect(fieldOf(container).getAttribute('dir')).toBe('rtl');
        cleanup();
    });
