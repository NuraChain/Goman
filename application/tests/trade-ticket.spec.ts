...
import { render } from './harness.ts';

import type { Market } from '../src/api.ts';

import TradeTicket from '../src/components/market/trade-ticket.azeroth';
...
describe('TradeTicket amount block', () =>
{
    ...
    it('renders the quick-add steps in the locale digits, pinned LTR so the plus leads', async () =>
    {
        useLocale().setLang('fa');
        const { container } = mount();

        (await import('node:fs')).writeFileSync('dump3.html', container.innerHTML);
