...
import Rail from '../src/components/ui/rail.azeroth';
...
describe('Rail', () =>
{
    ...
    {
        const { container } = render(() => Rail({ items: ROWS, itemKey: (row: Row) => row.id, label: 'Related', slotClass: 'w-10 shrink-0', children: (row: Row) => [row.id] }));
    ...
    {
        const { container } = render(() => Rail({ items: ROWS, itemKey: (row: Row) => row.id, label: 'Related', slotClass: 'shrink-0', children: (row: Row) => [row.id] }));

        const arrows = [...container.querySelectorAll<HTMLButtonElement>('button[aria-label]')].filter((button) =>
            button.getAttribute('aria-label')?.startsWith('Related:')
        );
    ...
    {
        const { container } = render(() => Rail({ items: ROWS, itemKey: (row: Row) => row.id, label: 'Categories', slotClass: 'shrink-0', dots: false, arrows: 'edge', children: (row: Row) => [row.id] }));
        ...
        const arrows = [...container.querySelectorAll<HTMLButtonElement>('button[aria-label]')].filter((button) =>
            button.getAttribute('aria-label')?.startsWith('Categories:')
        );
        expect(arrows).toHaveLength(2);
    });
});
